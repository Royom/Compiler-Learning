...
class ToIRVisitor : public ASTVisitor {
  ...
  Value *V;
public:
  StringMap<llvm::AllocaInst*> nameMap;
...
public:
  ...
  void visit(Factor &Node) override {
    if (Node.getKind() == Factor::Ident) {
      Value *VariableAddress = nameMap[Node.getVal()];
      //出错
       V = Builder.CreateLoad(VariableAddress);
      // 从变量地址加载其值
    } else { 
      int intval;
...
void visit(BinaryOp &Node) override {
  ...
  void visit(WithDecl &Node) override {
    ...
    for (auto Var : Node) {
      ...
      llvm::AllocaInst *VarAlloca = Builder.CreateAlloca(llvm::Type::getInt32Ty(Context), nullptr);
      Builder.CreateStore(Call, VarAlloca);
      nameMap[Var] = VarAlloca;
    }
...
void visit(AssignmentExpr &Node) override {
  // 获取左侧标识符
  llvm::StringRef VariableName = Node.getVariable();
  //遍历右边
  Node.getAssignedValue()->accept(*this);
  Value *AssignedValue = V;
  llvm::AllocaInst *variableAlloca;
 // 检查变量是否已经存在
  auto it = nameMap.find(VariableName);
  if (it != nameMap.end()) {// 如果变量已经存在，直接存入新的值
    variableAlloca = it->second;
    Builder.CreateStore(AssignedValue, variableAlloca);
  } else {
    // 如果变量不存在，分配空间并存入
    variableAlloca = Builder.CreateAlloca(llvm::Type::getInt32Ty(Context), nullptr, VariableName);
    Builder.CreateStore(AssignedValue, variableAlloca);
    // 将变量地址加入到 nameMap 中
    nameMap[VariableName] = variableAlloca;
  }
}
void visit(RootNode &Node) override {
    Node.getWithNode()->accept(*this);

    for (auto intDecl : Node.getIntDecls()) {
        intDecl->accept(*this);
    }

    Node.getMainExprNode()->accept(*this);
}
...
LLVMContext ToIRVisitor::Context;
} // namespace
...
void CodeGen::compile(AST *Tree) {
  ...
  // llvm::StringRef FileName = "out_ir.txt";
  // std::error_code EC;
